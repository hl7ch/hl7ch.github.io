...
The fast path for maintaining the fhir.ch IG catalog. If you just need to start a ballot cycle, hide an IG, or add a workgroup attribution, you're in the right place. For the full data contracts and every curation knob, see [WEBSITE_README.md](WEBSITE_README.md). For the IG publication pipeline see [README.MD](README.MD).
...
Append a fully-specified record to `EXTRA_IGS` in `js/load-data.js`. The Swissnoso and MedNet Interface entries are the working templates — copy one and replace the fields. This is a stopgap; the proper fix is to publish the IG through the publication workflow (see [README.MD](README.MD)) so it lands in `package-registry.json` natively.

## 5. When to read the full README

Open [WEBSITE_README.md](WEBSITE_README.md) when you need:
