# fhir.ch landing page

Reference for maintaining the [fhir.ch](https://www.fhir.ch) landing page. For the IG publication pipeline (workflows, `publication-request.json`, etc.) see [README.MD](README.MD); this file covers only the page itself.

> **New to the site?** Start with [WEBSITE_QUICKSTART.md](WEBSITE_QUICKSTART.md) — the five most common maintenance recipes in one page. This README is the deeper reference.

The page is a single-page app — three files (`index.html`, `js/load-data.js`, `js/app.js`) plus a stylesheet. The IG catalog is **fetched at page-load time** same-origin from this repo's root; there is no build step, no Node, no Python, no CI runtime, no database. Any static host that serves this repo (the existing reverse proxy mirroring it to fhir.ch, GitHub Pages, `python3 -m http.server`) works.
...
No internet connection required — the catalog data lives in this repo (`package-registry.json` + `ig/{slug}/package-list.json`) and is fetched same-origin.

## Where the data comes from

Two files served same-origin from the repo root:

| File | URL |
|---|---|
| Registry | `./package-registry.json` |
| Per-IG list | `./ig/{slug}/package-list.json` |

Both are produced by the IG publisher (see [README.MD](README.MD)) and committed to this repo by the publication workflow. No external fetch, no CORS, no proxy.
...
- **`WG_*` shorthands** — reusable workgroup objects. Add new ones rather than inlining `{name, url}` literals. The names follow the official project-group list on the [HL7 CH Technisches Komitee page](https://www.hl7.ch/technisches-komitee/): *Arbeitsgruppe FHIR*, *Joint Venture Arbeitsgruppe Radiologie*, *Joint Venture Laborprojekt FAMH*, *Joint Venture Arbeitsgruppe «Austauschformate EPD»*, *Joint Venture Arbeitsgruppe HUSKY*. Keep them in sync with that page rather than inventing per-IG wording. `WG_EPD` ("Joint Venture Arbeitsgruppe EPD", used by CH IPS and CH EPREG) is **not** on that list and is carrying a `TODO` until HL7 CH confirms which group owns those two.
