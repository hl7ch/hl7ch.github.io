# Guidance Others - CH EMED EPR v3.0.0
...
The patient body weight can be specified by adding a [Body Weight Observation](StructureDefinition-ch-emed-epr-observation-body-weight.md) to the Bundle in the entry "ObsBodyWeight". It must be referenced from the Composition too, in the section "vitalSigns".
...
The `id` of the practitionner is `FamilienHausarzt`. This value is used in the `fullUrl` for internal references within the FIHR document (see also [compositions guidance](guidance_compositions.md)).
