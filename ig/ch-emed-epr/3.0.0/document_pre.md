# Document Pre - CH EMED EPR v3.0.0
...
* [Document PRE](StructureDefinition-ch-emed-epr-document-medicationprescription.md)
* [Composition PRE](StructureDefinition-ch-emed-epr-composition-medicationprescription.md)
* [MedicationRequest (PRE)](StructureDefinition-ch-emed-epr-medicationrequest.md)
...
The prescription validity period (`MedicationRequest.dispenseRequest.validityPeriod.(start|end)`) indicates the period over which the prescription can be dispensed (NOT the period over which the medication is to be taken by the patient). The validity period in a medication request resource of a prescription document is now optional, as well as its start and end time elements if the validity period is specified:
