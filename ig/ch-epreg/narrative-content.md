# Narrative Content - CH EPREG (R4) v1.0.0
...
The requirements of CH EPREG for narrative content are identical to those of [CH IPS](https://fhir.ch/ig/ch-ips/narrative-content.html) and are based on the guidelines [FHIR Clinical Documents](#fhir-clinical-documents) and [HL7 IPS-UV](#hl7-ips-uv), which are explained below.
 ...
 **Ongoing FHIR implementations will influence future guidance, such as the FHIR Clinical Document Implementation Guide, about the content and display of narrative content, which is required in the IPS Composition. We encourage implementers to monitor this topic and provide feedback through HL7 engagement, issue logging, and community discussion.** 
 ...
 Please add your feedback on this topic for CH EPREG via the 'Propose a change'-link in the footer of the page. 

 

### Example

The following are snippets for `Composition.text` and `Composition.section.text` from the [UC 1: Pregnancy Passport](Bundle-UC1-Document.json.md) example:

`Composition.text` containing document metadata:

 

`Composition.section:pregProgress.text` containing attested narrative:
