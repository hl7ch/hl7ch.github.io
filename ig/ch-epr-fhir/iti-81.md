# Retrieve ATNA Audit Event [ITI-81] - CH EPR FHIR (R4) v5.0.0
...
1. `<query>`shall include the entity.identifier as defined in[Additional ATNA Search Parameters](#additional-atna-search-parameters)and may include additional ATNA Search parameters. If entity.identifier is not included an HTTP response code 400 - Bad Request shall be returned.
...
The returned AuditEvent FHIR resources in the Bundle shall conform the CH:ATC AuditEvent profile, see [Volume 3 - CH:ATC Audit Event Content Profiles](volume3.md).
...
1. Use an extended access token defined in[IUA](iti-iua.md)conveyed as defined in the[Incorporate Access Token [ITI-72]](https://profiles.ihe.net/ITI/IUA/index.html#372-incorporate-access-token-iti-72)transaction.
...
An audit event as specified in [Access Audit Trail Content Profile](volume3.md#access-audit-trail-content-profile) shall be returned by a query to Patient Audit Record Repository after the Patient Audit Record Repository has been queried by a Patient Audit Consumer.
