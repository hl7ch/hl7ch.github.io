# Mobile Privacy Policy Retrieve [PPQ-5] - CH EPR FHIR (R4) v5.0.0
...
The **Policy Repository** shall support the following search parameters on the [Consent](StructureDefinition-PpqmConsent.md) resource:

| | | | |
| :--- | :--- | :--- | :--- |
| patient:identifier | token | Consent.patient.identifier | For querying by patient ID (EPR-SPID)`Consent?patient:identifier=urn:oid:2.16.756.5.30.1.127.3.10.3|[epr-spid]` |
| identifier | token | Consent.identifier | For querying by policy set ID`Consent?identifier=[uuid]` |
...
The PPQ-5 response SHALL be created according to the section [3.1.0.9](https://hl7.org/fhir/R4/http.html#search) of the FHIR R4 specification. If the response body is a Bundle, then it SHALL comply to the [PpqmRetrieveResponseBundle](StructureDefinition-PpqmRetrieveResponseBundle.md) profile.
...
The **Policy Consumer** SHALL record an audit event according to [CH Audit Event for [PPQ-5] Policy Consumer](StructureDefinition-ChAuditEventPpq5Consumer.md).

The **Policy Repository** SHALL record an audit event according to [CH Audit Event for [PPQ-5] Policy Repository](StructureDefinition-ChAuditEventPpq5Repository.md).
