...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <!-- 
    This file contains just the constraints for the profile IHE_PMIR_Bundle
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Bundle/f:entry/f:resource">
      ...
      <sch:assert test="not(f:entry/f:request) or (f:type/@value = 'batch') or (f:type/@value = 'transaction') or (f:type/@value = 'history')">entry.request mandatory for batch/transaction/history, otherwise prohibited</sch:assert>
      <sch:assert test="not(f:entry/f:response) or (f:type/@value = 'batch-response') or (f:type/@value = 'transaction-response') or (f:type/@value = 'history')">entry.response mandatory for batch-response/transaction-response/history, otherwise prohibited</sch:assert>
      ...
      <sch:assert test="not(f:type/@value = 'document') or exists(f:timestamp/@value)">A document must have a date</sch:assert>
      <sch:assert test="not(f:type/@value='document') or f:entry[1]/f:resource/f:Composition">A document must have a Composition as the first resource</sch:assert>
      <sch:assert test="not(f:type/@value='message') or f:entry[1]/f:resource/f:MessageHeader">A message must have a MessageHeader as the first resource</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
